...
class EditItemState extends ChangeNotifier {
  ...
  ItemResponse itemToEdit = ItemResponse();
  final client = Client();
  ...
  ImageProvider<Object> get pickedImage {
    if (_imageFile != null) {
      ...
      if (itemToEdit.itemImageURL != null &&
          itemToEdit.itemImageURL != defaultImage) {
        return NetworkImage(itemToEdit.itemImageURL!);
      }
  ...
  Future<bool> updateItem() async {
    try {
      UpdateItemRequest updatedItem = UpdateItemRequest(
        itemId: itemToEditID,
        ...
        imagePath: imageFile != null
            ? imageFile!.path
            : (imageFile == null && itemToEdit.itemImageURL != null)
                ? itemToEdit.itemImageURL
                : defaultImage,
      );
      notifyListeners();
      return await _itemService.updateItem(updatedItem);
    } catch (error) {
  ...
  void processImage(String itemImageURL) async {
    final InputImage inputImage = InputImage.fromFilePath(itemImageURL);
